...
pub fn run_hiper(ctx: ExtEventSink, token: String, use_tun: bool, _debug_mode: bool) -> DynResult {
    ...
    if cert_path.is_file() {
        ...
        if let Ok(res) = tinyget::get("https://dreamelink-1300179650.cos.ap-nanjing.myqcloud.com/point.yml").send() {
        ...
        let res = tinyget::get(format!("https://zs.dreame.top/token/{}.yml", token))
            .send()
